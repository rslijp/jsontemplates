...
public class DescribeModelHelper {
    ...
    private ModelPropertyDescription describeModelProperty(DefinitionRegistryEntry entry, TemplateDescription template, Map<String, ModelDescription> seen) {
    ...
    private int describeSubModel(Class modelType, DefinitionRegistryEntry[] entries, TemplateDescription template, Map<String, ModelDescription> seen) {
        String key = modelType.getName();
        if(seen.containsKey(key)) return seen.get(key).getId();
