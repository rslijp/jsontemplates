...
public class TypeCheckException extends RuntimeException {
    ...
    public static TypeCheckException typeError(IExpressionType expectedType, IExpressionType actualType, String argumentName) {
        var uxType = expectedType.baseType()== Types.GENERIC? GenericType.inferT(actualType) :actualType;
        return new TypeCheckException(String.format("Expected argument %s to be %s but found %s", argumentName, expectedType.getType(), uxType.getType()));
    }
    ...
    public static TypeCheckException wrongModel(String expectedModelType, String actualModelType, int argumentIndex) {
        return new TypeCheckException(String.format("Expected argument %d to be model %s but found %s", argumentIndex, expectedModelType, actualModelType));
    }

    public static TypeCheckException slotMismatch(String name, ISlotPattern pattern, INode actual) {
        return new TypeCheckException(String.format("Slot '%s' only accepts '%s' but found '%s'", name, pattern.getDescription(), actual.getClass().getSimpleName()));
    }
