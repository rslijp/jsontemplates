...
import nl.softcause.jsontemplates.collections.*;
...
public class DefinitionRegistry {
    ...
    private static Pattern REPLACE_INDEX = Pattern.compile("((\\[[0-9]+\\])|(\\([a-zA-Z0-9àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ \\(\\)\\.,'\\-]+\\)))");
    private static Pattern ENDS_WITH_INDEX = Pattern.compile("((\\[[0-9]+\\])|(\\([a-zA-Z0-9àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ \\(\\)\\.,'\\-]+\\)))$");
    ...
    void addDefintion(String name, IExpressionType type, DefinitionRegistry nested, boolean readeable, boolean writerable) {
    ...
    Object read(Object source, String name){
        ...
        try {
            ...
            if(endsWithIndex) type=type.baseType();
    ...
    public DefinitionRegistryEntry[] getDefinitions() {
        return model.values().stream().sorted(Comparator.comparing(DefinitionRegistryEntry::getName)).toArray(DefinitionRegistryEntry[]::new);
    }
}
