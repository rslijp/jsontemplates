...
public class ModelException extends RuntimeException {
    ...
    static ModelException primitiveCantHaveProperty(IExpressionType type, String property, String fullPath) {
        return new ModelException(String.format("Primitive object(%s) can't have property '%s'. Part of accessing ('%s')",type.getType(), property, fullPath));
    }
    ...
    static ModelException invocationErrorDuringGet(String path, Class<?> type, Exception ex) {
        return new ModelException(String.format("Error invoking '%s' on %s. Error %s",path, type.getSimpleName(), ex.getMessage()), ex);
    }
    ...
    public static ModelException wrongType(String path, IExpressionType expectedType, IExpressionType actualType) {
        return new ModelException(String.format("Expected value of type %s but found %s for %s", expectedType.getType(), actualType.getType(), path));
    }

    public static ModelException wrongType(String path, Class<?> expectedType, Class<?> actualType) {
        return new ModelException(String.format("Expected value of type %s but found %s for %s", expectedType.getSimpleName(), actualType.getSimpleName(), path));
    }
