...
public class For extends ReflectionBasedNodeWithScopeImpl<For.ForScope> {
    ...
    private static TemplateModelException countUpBoundaryException(Long start, Long end) {
        return new TemplateModelException(String.format("Case counting up(step>0) start value(%d) must be less then the end value(%d)", start, end));
    }

    private static TemplateModelException countDownBoundaryException(Long start, Long end) {
        return new TemplateModelException(String.format("Case counting down(step<0) start value(%d) must be greater then the end value(%d)", start, end));
    }
